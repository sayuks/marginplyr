...
```{r duckdb, eval = requireNamespace("DBI", quietly = TRUE) && requireNamespace("duckdb", quietly = TRUE)}
...
[`quartabs`](https://sayuks.github.io/quartabs/) so regions and totals become
Quarto tabs:
...
by `grouping_sets()` and the Cartesian product made by `grouping_spec()`.

...
Where a local summary returns a data frame, its columns enter dplyr's summary
mask immediately and the assigned name does not appear.
...
summary changes its result shape whenever its value is a data frame, and a
local branch must pass that value through an unnamed summary. Which summaries
those are is a question about the value's type, and two expressions one call
apart fall on opposite sides of it:
...
So the summary receives an Assigned summary name during planning. For a local
branch, its rewritten expression is passed to dplyr as an unnamed summary that
returns a data frame. An actual data frame retains its own columns; a scalar
is put into a one-column frame under the Assigned summary name. dplyr expands
both while evaluating the summary, making their columns available to later
summaries in the same data mask. The former expansion after the branch had
finished did not preserve that ordering (#619).
...
|---|---|---|
| `data.frame`, `data.table` | expanded | expanded while the branch summarizes |
...
[ADR 0020](0020-ask-before-reading-a-lazy-input.md) is not engaged. The local
value is already being evaluated by dplyr and sends no query; the guard is what
keeps it away from every input that could be asked one.

## Where the expansion runs

`summarize_margin_union()`, per branch, before handing its dots to
`summarize_margin_branch()`. Only local data frames use
`wrap_assigned_local_summaries()`; lazy branches retain their named dots. dplyr
then expands the local frame in the summary mask, before the next dot runs.
...
`duckdb` and `postgres` kinds alone — both lazy.

The carrier is `new_summary_arguments()`, which holds a third vector parallel to
...
`check_summary_output_names()` reads the branch result after dplyr expands its
unnamed frames. An inner name equal to a grouping dimension is refused, and
one equal to the caller's `.id` is refused, in the wording those two already
use. No pre-execution reading knows what the frame will hold.

The composite is asked rather than its parts. The branch's internal key names
are present while it runs, and `set_id_is_internal` covers a Grouping set
identifier `add_grouping_set_id()` has yet to write. Asking only the other two
questions would leave an inner name equal to that column silently overwritten.
...
`stage_margin_summaries()` asks it once over the staged result instead. The
caller's `.id` is renamed onto the result at the end, so an unasked collision
loses the summary rather than refusing it.

## Consequences

dplyr writes the expanded columns into its own result, so the result's class
and duplicate-name behavior follow dplyr's summary.
[ADR 0016](0016-delegate-result-class-and-attributes-to-dplyr.md) governs the
result's class and attributes.

...
the caller wrote, and it exists only while the query is built. On a local
input, a data-frame value expands into dplyr's summary mask before later
summaries run, and the assigned name does not appear. A scalar retains that
name. A name the caller wrote is not one of these and packs, as it does under
dplyr. Which unnamed summaries take one is decided by spelling, and whether
one expands is decided from its value while the summary runs; ADR 0028 is
authoritative for why the two are settled in different places.

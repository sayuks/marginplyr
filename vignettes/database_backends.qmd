...
For now, direct `compute()` of a SQLite Margin result using marginplyr's
dedicated type or order boundary raises an error before writing. Use
`collect()` to retrieve that result while the destination fix in #661 is in
progress. Ordinary dbplyr queries and Margin results outside that boundary
can still use `compute()`.

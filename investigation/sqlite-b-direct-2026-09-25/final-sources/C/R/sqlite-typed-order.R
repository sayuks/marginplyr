...
collect.marginplyr_sqlite_typed_result <- function(x, ..., n = Inf,
  ...
  if (TRUE) {
    for (name in names(attr(x, "marginplyr_declared_types"))) {
      ...
      out[[name]] <- switch(type, integer = as.integer(out[[name]]), double = as.double(out[[name]]), character = as.character(out[[name]]))
    }
  }
  out
}
...
#' @noRd
compute.marginplyr_sqlite_typed_result <- function(x, ..., in_transaction = FALSE) {

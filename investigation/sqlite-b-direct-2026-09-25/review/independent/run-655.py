...
out=root/'review/independent'
cases=list(product(('on','off'),('implicit','explicit'),('sorted','unsorted'),('collect','compute')))
def run(c):
 name='655-'+'-'.join(c)
 with (out/(name+'.log')).open('w') as log:
  p=subprocess.run(['Rscript','--vanilla',str(out/'655-fresh.R'),str(root/'variants/B'),*c,str(out/(name+'.rds'))],stdout=log,stderr=subprocess.STDOUT)
